<?php
  ...
  class Query extends Base {
      ...
      if ($field == self::FIELD_VALUE) {
        foreach ($value as $k => $val) {
          if (!is_string($val) and !is_numeric($val)) {
            throw new \Fiv\Tokenizer\Exception('Invalid value. Must be string');
          }
    ...
    public function checkToken(Token $token) {
      # check type
      if (!$this->validate(self::FIELD_TYPE, $token->type())) {
        return false;
      }

      # check value
      if (!$this->validate(self::FIELD_VALUE, $token->value())) {
        return false;
      }

      # check line
      if (!$this->validate(self::FIELD_LINE, $token->line())) {
        return false;
      }
